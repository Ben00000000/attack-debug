...
function preload() {
    ...
    this.load.spritesheet('attack', 'attack.png', { frameWidth: 120, frameHeight: 138 });
...
this.anims.create({
    key: 'attack',
    frames: this.anims.generateFrameNumbers('attack', { start: 7, end: 7, first: 0 }),
    frameRate: 22,
    repeat: -1  // Set repeat to -1 for continuous looping
});
...
if ((cursors.up.isDown || jumpButtonPressed) && player.body.onFloor()) {
   ...
   if (shootButtonPressed && game.getTime() - lastPlayerLaserTime > 0) {
        ...
        lastPlayerLaserTime = game.getTime();
         ...
         if (cursors.space.isDown && game.getTime() - lastPlayerLaserTime > 0) {
               // Call a function to shoot a player laser
               shootPlayerLaser();
...
function shootPlayerLaser() {
    ...
    player.anims.play('attack', true);
     ...
     const laserX = player.flipX ? player.x - 80 : player.x + 80;
       ...
       var laser = lasers.create(laserX, player.y - 40, 'purpleLaser');
    ...
    // Set a timeout to stop the 'attack' animation after a certain time (adjust as needed)
    setTimeout(() => {
        player.anims.stop('attack');
        player.anims.play('idle', true); // Assuming 'idle' is your idle animation
    }, 0); // Adjust the duration of the 'attack' animation
